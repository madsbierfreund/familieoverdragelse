...
describe('explain', () => {
  ...
  it('skriver regnestykket bag beregningsmassen og statuslinjen', () => {
    ...
    expect(t.estateMass).toContain('8.000.000 ÷ 5 = 1.600.000 kr. pr. barn');

    expect(t.status).toContain('overstiger hendes arvelod med 2.102.600 kr.');
    expect(t.status).toContain('4.297.400 ÷ 4 = 1.074.350 kr.');
  });
  ...
  it('forklarer et dækket forskud, hvor datteren indbetaler', () => {
    ...
    expect(t.status).toBe(
      'Datterens forskud på 5.702.600 kr. er mindre end hendes arvelod på ' +
        '6.600.000 kr. Hun arver differencen på 897.400 kr., som modregnes ' +
        'i restgælden på 2.297.400 kr. Hun indbetaler derfor 1.400.000 kr. ' +
        'til boet. Hver søskende får 6.600.000 kr. Det gælder med og uden ' +
        'udligningsaftale.',
    );
  });
  ...
  it('forklarer et dækket forskud, hvor datteren får udbetalt', () => {
    ...
    expect(t.status).toContain('Hun får derfor 1.600.000 kr. udbetalt.');
    expect(t.status).toContain('Hver søskende får 9.600.000 kr.');
  });

  it('forklarer et dækket forskud, hvor beløbene går lige op', () => {
    const t = texts({
      marketValue: 12_000_000,
      otherAssets: 32_000_000,
      ownFinancing: 4_000_000,
    });
  ...
  it('nævner ikke det gamle antal søskende', () => {
    ...
    expect(INTRO).toContain('datteren og fire søskende');
    for (const text of [INTRO, ...Object.values(t)]) {
      expect(text).not.toContain('tre søskende');
    }
  });
});
