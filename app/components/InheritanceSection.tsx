...
export default function InheritanceSection({
  ...
  const [raw, setRaw] = useState<Record<FieldName, string>>(() => ({
    ...
    otherAssets: String(values.otherAssets),
    ownFinancing: String(values.ownFinancing),
  }));
          ...
          return (
            <div className={styles.field} key={field.name}>
              ...
              <input
                ...
                onChange={(event) => update(field.name, event.target.value)}
              />
              <p className={`${styles.explanation} ${styles.inputExplanation}`}>
                {INPUT_EXPLANATIONS[field.name]}
              </p>
            </div>
          );
        })}
